...
def get_ffmpeg_path():
    """Возвращает путь к ffmpeg.exe в папке codecff"""
    # Определяем путь к папке codecff относительно текущего скрипта
    script_dir = os.path.dirname(os.path.abspath(__file__))
    codecff_dir = os.path.join(script_dir, "codecff")
    ffmpeg_path = os.path.join(codecff_dir, "ffmpeg.exe")
    
    # Проверяем, существует ли файл ffmpeg.exe по этому пути
    if os.path.exists(ffmpeg_path):
        return ffmpeg_path
    else:
        return None

def check_ffmpeg(ffmpeg_executable):
    """Проверяет наличие ffmpeg по указанному пути"""
    if not ffmpeg_executable:
        return False
    try:
        # Используем конкретный путь к ffmpeg.exe
...
def merge_files(ffmpeg_executable, video_file, audio_file, output_file):
    """Объединяет видео и аудио файлы, используя указанный путь к ffmpeg"""
    command = [
        ffmpeg_executable, # Используем найденный путь
        "-i", video_file,
        "-i", audio_file,
        "-c:v", "copy",
...
def main():
    ...
    # Получаем путь к ffmpeg.exe
    ffmpeg_path = get_ffmpeg_path()
    
    # Проверяем наличие ffmpeg по указанному пути
    if not check_ffmpeg(ffmpeg_path):
        print("❌ FFmpeg не найден по пути ./codecff/ffmpeg.exe!")
        print("Пожалуйста, убедитесь, что файл ffmpeg.exe находится в папке 'codecff' рядом со скриптом.")
        print("Скачать ffmpeg можно здесь: https://ffmpeg.org/download.html")
    ...
    # Объединяем файлы, передавая путь к ffmpeg
    success, message = merge_files(ffmpeg_path, video_file, audio_file, output_file)
